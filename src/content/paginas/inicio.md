---
route: /
title: Aprender. Crear. Ser felices.
description: >-
  Conoce el CEDHU en Sogamoso: preescolar, primaria y bachillerato con Pedagogía de la Felicidad,
  valores, inglés, ciencia y robótica.
heroTitle: |-
  Aprender.
  Crear.
  Ser felices.
heroDescription: |-
  El conocimiento nos abre caminos.
  Los valores nos enseñan a recorrerlos.
heroEyebrow: Tecnología + academia + humanidad
heroImage: aulaCedhu
hero:
  context: >-
    En el CEDHU, la Pedagogía de la Felicidad conecta la excelencia académica con el desarrollo del
    SER.
  admissionsLabel: Quiero conocer el CEDHU
  educationLabel: Nuestra educación
  collageLabel: EL APRENDIZAJE SE VIVE
  photoCaption: Ideas propias.
  photoCaptionEmphasis: Posibilidades compartidas.
  campusImage: instalacionesCedhu
  campusCaption: Un lugar para crecer.
  playImage: convivenciaCedhu
  playCaption: Aprender a convivir.
  insetImage: inicial
  insetLabel: La curiosidad empieza aquí
  footerDescription: Un colegio para crecer en todas las dimensiones.
  discoverLabel: Descubre el CEDHU
  levelsLabel: INICIAL · PRIMARIA · BACHILLERATO
campusModel:
  eyebrow: Nuestros espacios
  title: Espacios para aprender, crear y crecer.
  description: Un lugar diseñado para acompañar el aprendizaje, la creatividad, la convivencia y el desarrollo de nuestros educandos.
  label: Maqueta ilustrativa del CEDHU con sus edificios, cancha y patio central, vistos desde arriba.
  caption: Maqueta ilustrativa de nuestras instalaciones.
# Síntesis del Manual de Convivencia, cap. II, apartados 4–10 (pp. 33–37)
# y anexo 1, Política de Inclusión (pp. 174–179). Son propósitos formativos,
# no garantías de resultados individuales ni un catálogo de servicios.
whyCedhu:
  eyebrow: ¿Por qué elegir el CEDHU?
  titleStart: Crecer, con todo
  titleEmphasis: lo que eres.
  introduction: >-
    Cada educando puede conocerse, convivir y dar forma a sus ideas,
    mientras desarrolla habilidades que lo acompañen en la vida.
  image: educandaAulaCedhu
  photoCaption: Un educando, muchas posibilidades de crecer.
  experiences:
    - title: Aprender con bienestar
      description: >-
        Encontrar gusto en aprender, fortalecer sus valores y cuidar la relación
        consigo mismo y con los demás, buscando bienestar.
    - title: Crecer en todas sus dimensiones
      description: >-
        Desarrollar sus conocimientos, cuidar su cuerpo y comprender sus emociones,
        sin separar sus vínculos, su pensamiento y su vida espiritual.
    - title: Preguntar, crear y descubrir
      description: >-
        Hacer preguntas e investigar. Desarrollar su pensamiento crítico y su creatividad
        para analizar situaciones, explorar respuestas y construir soluciones.
    - title: Convivir desde el respeto
      description: >-
        Reconocer sus derechos y los de los demás. Dialogar, mediar y construir acuerdos,
        con respeto por las diferencias.
    - title: Decidir y construir su camino
      description: >-
        Tomar decisiones, cumplir compromisos y comprender sus consecuencias:
        pasos para ejercer su libertad con responsabilidad y construir su propio camino.
    - title: Crecer desde su individualidad
      description: >-
        Aprender desde sus capacidades, intereses y necesidades, con apoyos que reconocen
        su individualidad y favorecen su participación junto a los demás.
  linkLabel: Conoce la propuesta educativa
  linkHref: /oferta-educativa/
humanFormation:
  image: horizonte
  photoCaption: EL SER, EN EL CENTRO.
  eyebrow: Nuestra manera de educar
  titleStart: La felicidad también
  titleEnd: se
  titleEmphasis: aprende.
  introduction: >-
    Enseñar y aprender con bienestar. Esa es la búsqueda de la Pedagogía de la Felicidad: fortalecer
    los valores del SER y hacer del conocimiento una experiencia cercana.
  supportDescription: >-
    La hora de valores, los talleres y el Student Emotional Support Room acompañan el desarrollo
    humano y la sana convivencia.
  linkLabel: Conoce la Pedagogía de la Felicidad
  linkHref: /nosotros/pedagogia-de-la-felicidad/
identityIntro:
  eyebrow: El centro siempre es humano
  titleStart: Una educación que conecta
  titleEnd: lo que sabes con
  titleEmphasis: quien eres.
  introduction: >-
    Somos el Centro de Desarrollo Humano, en Sogamoso. Una comunidad que cree en aprender con rigor,
    crear con curiosidad y convivir con respeto.
  familyDescription: >-
    La familia, el educando y la institución comparten un propósito: crecer de manera integral, con
    valores que se viven todos los días.
  linkLabel: Esta es nuestra esencia
  humanPillar: El SER como punto de partida
  knowledgePillar: El saber como descubrimiento
  communityPillar: Convivir como aprendizaje
  linkHref: /nosotros/
newsSection:
  eyebrow: Pasa en el CEDHU
  title: Una comunidad que se vive.
  linkLabel: Ver toda la actualidad
  linkHref: /noticias/
quickAccess:
  eyebrow: A un paso
  titleStart: Tu día a día,
  titleEnd: más fácil.
  links:
    - label: Ingresar a Q10
      detail: Plataforma académica
      href: https://site2.q10.com/login?ReturnUrl=%2F&aplentId=04dd32ac-1f6a-46d0-b388-df88d12b739d
      external: true
    - label: Consultar documentos
      detail: Información para familias
      href: /documentos/
      external: false
    - label: Conocer las lúdicas
      detail: Talentos en movimiento
      href: /ludicas/
      external: false
    - label: Hablar con el CEDHU
      detail: Estamos para orientarte
      href: /contacto/
      external: false
technology:
  eyebrow: Ciencia + tecnología + creatividad
  titleStart: La curiosidad
  titleEnd: tiene
  titleEmphasis: dónde crecer.
  introduction: Aprender también es hacerse preguntas, ensayar una idea y descubrir qué podemos crear juntos.
  image: laboratorio
  photoCaptionStart: Dentro del CEDHU
  photoCaptionEnd: Laboratorio de ciencias
  linkLabel: Explora nuestra propuesta
  areas:
    - name: Ciencia que se experimenta
      description: Laboratorios de física y química
      href: /oferta-educativa/ciencia-tecnologia/
    - name: Ideas que toman forma
      description: Sistemas, programación y robótica
      href: /oferta-educativa/robotica/
    - name: Conexiones que enseñan
      description: STEAM Mathematics Classroom
      href: /oferta-educativa/ciencia-tecnologia/#enfoques
  linkHref: /oferta-educativa/ciencia-tecnologia/
---
